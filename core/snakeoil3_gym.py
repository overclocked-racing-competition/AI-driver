#!/usr/bin/python
# snakeoil3 — SCR (Simulated Car Racing) UDP client for TORCS.
# Based on snakeoil.py by Chris X Edwards <snakeoil@xed.ch>
# (http://scr.geccocompetitions.com/), ported to Python 3.
# ServerState (S) holds incoming telemetry; DriverAction (R) holds the reply;
# both expose a data dict `d`. See http://xed.ch/help/torcs.html for the protocol.
import socket
import sys
import getopt
import os
...
def _relaunch_torcs_windows():
    # Force-restart a crashed/hung TORCS on Windows (kill -> launch -> autostart).
    #
    # TORCS is unstable over long unattended runs. The connection loops call this
    # when the server has been unresponsive for a while, so a worker self-heals
    # instead of hanging forever printing 'Waiting for server'.
    try:
        from config import Config
        torcs_dir = Config.torcs.torcs_dir
        torcs_exe = Config.torcs.torcs_exe
    except Exception:
        torcs_dir = r"D:\torcs\torcs"
        torcs_exe = r"D:\torcs\torcs\wtorcs.exe"
    print("[snakeoil] Force-relaunching TORCS (kill + launch + autostart)...")
    os.system("taskkill /IM wtorcs.exe /F >nul 2>&1")
    os.system("taskkill /IM torcs.exe /F >nul 2>&1")
    time.sleep(1.0)
    os.system('start "" /D "%s" "%s" -nolaptime' % (torcs_dir, torcs_exe))
    time.sleep(1.5)
...
class Client():
    ...
    def setup_connection(self):
        ...
        n_fail = 25   # ~25s to connect before giving up (tolerates slow startup under many workers)
        nt_wait = 0   # consecutive 1s timeouts on Windows (for self-heal relaunch)
        while True:
            ...
            try:
                ...
                if os.name == 'nt':
                    nt_wait += 1
                    if nt_wait == 1 or nt_wait % 5 == 0:
                        print("--> [WINDOWS] Oczekuje na start wyścigu (waiting %ds)..." % nt_wait)
                    # Self-heal: if TORCS hasn't answered in ~20s, force-relaunch it.
                    # This is the fix for the overnight hang — previously this branch
                    # looped forever and the relaunch code below was unreachable.
                    if nt_wait >= 20:
                        _relaunch_torcs_windows()
                        nt_wait = 0
                    time.sleep(1.0)
                    continue

                # Multi-worker safe: NEVER global-`pkill torcs` or open a GUI TORCS
                # here — that kills every OTHER worker's race and cascades into an
                # endless relaunch/window storm. Fail fast; the per-trial driver
                # relaunches THIS worker's TORCS with a scoped kill.
                n_fail -= 1
                if n_fail < 0:
                    raise ConnectionError("scr_server not reachable on port %d" % self.port)
    ...
    def get_servers_input(self):
        ...
        sockdata= str()
        nt_fail = 0   # consecutive 1s timeouts mid-race (for self-heal relaunch)

        while True:
            try:
                # 1. Receive AT LEAST ONE server data packet (blocking)
                sockdata, addr = self.so.recvfrom(data_size)
                
                # 2. Drain the OS buffer to catch up to real-time (non-blocking)
                self.so.setblocking(0)
                while True:
                    try:
                        latest_data, _ = self.so.recvfrom(data_size)
                        sockdata = latest_data
                    except socket.error:
                        break  # Buffer is empty
                self.so.setblocking(1)
                ...
                # Self-heal: if TORCS dies mid-race and stops sending data, force-
                # relaunch it and re-handshake instead of looping '.' forever.
                if os.name == 'nt':
                    nt_fail += 1
                    if nt_fail >= 20:
                        print("\n[snakeoil] No data ~20s mid-race — TORCS likely crashed.")
                        _relaunch_torcs_windows()
                        try:
                            self.setup_connection()   # re-handshake (also self-heals)
                        except Exception:
                            pass
                        nt_fail = 0
