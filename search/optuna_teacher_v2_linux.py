# WSL Optuna worker for the v2 lookahead-PID teacher (legacy).
# Same headless torcs -r infra as the v4-v6 drivers.
import os
import sys
import math
import time
...
from search.optuna_teacher_linux import launch_torcs, kill_torcs, install_practice_xml, TORCS_HOME
from agents.teacher_controller_v2 import TeacherController, sample_params

DAGGER_BASELINE_LAP = 106.96
INF = float("inf")


def evaluate_v2(params, port=3001, n_laps=1, timeout_s=300.0):
    # Drive the v2 controller against a live TORCS via snakeoil; return lap stats.
...
def main():
    ap = argparse.ArgumentParser(description="WSL Optuna driver for the V2 lookahead-PID teacher")
    ...
    storage_url = args.storage or os.environ.get("OPTUNA_STORAGE") or "sqlite:///optuna_teacher_v2.db"
    ...
    print(f"\n{'='*60}")
    print(f"  V2 lookahead-PID driver  [storage={backend}] [launch={args.launch}]")
    ...
    while global_done < args.n_trials:
        ...
        try:
            result = evaluate_v2(params, port=args.port, n_laps=args.n_laps, timeout_s=args.timeout_s)
        except Exception as e:
