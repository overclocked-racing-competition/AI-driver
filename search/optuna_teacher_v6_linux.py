# WSL Optuna worker for the v6 teacher (headless torcs -r, local SQLite study).
# Usage: python3 optuna_teacher_v6_linux.py --study-name teacher_v6_ow1 \
#            --storage sqlite:////home/user/optuna_ow1.db --port 3001 --n-trials 1000000
...
from agents.teacher_controller_v6 import TeacherController, sample_params
...
def evaluate(params, port, n_laps, timeout_s):
    ...
    last, step, last_prog_step, last_dist = None, 0, 0, 0.0
    try:
        ...
        while time.time() - t0 < timeout_s and len(laps) < n_laps:
            ...
            elif step - last_prog_step > 200:      # ~4s stuck (wedged on barrier) → bail
...
def main():
    ...
    url = args.storage or os.environ.get("OPTUNA_STORAGE") or "sqlite:///optuna_teacher_v6.db"
    ...
    print(f"\n{'='*58}\n  V6 apex-line driver  [storage={backend}]")
