# Create and seed a fresh Optuna study in Postgres (legacy; see new_study.py).
...
ap.add_argument("--study-name", required=True, help="new study name, e.g. teacher_wp_3523")
args = ap.parse_args()

url = os.environ.get("OPTUNA_STORAGE")
if not url or "postgres" not in url:
    print("ERROR: OPTUNA_STORAGE not set to a postgres URL in this process.")
    sys.exit(1)
...
print(f"[pg] study '{args.study_name}' ready. completed trials: {done}")
